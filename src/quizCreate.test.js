...
describe('adminQuizCreate', () => {
    ...
    test('Invalid name - too short', () => {
        ...
        expect(adminQuizCreate(authUserId.authUserId, 'Ab', 'A test quiz')).toEqual(ERROR);
    });

    test('Invalid name - too long', () => {
        const authUserId = adminAuthRegister('swastik@example.com', 'password123', 'swastik', 'mishra');
        const longName = 'A'.repeat(31);
        expect(adminQuizCreate(authUserId.authUserId, longName, 'A test quiz')).toEqual(ERROR);
    });

    test('Invalid name - non-alphanumeric characters', () => {
        const authUserId = adminAuthRegister('swastik@example.com', 'password123', 'swastik', 'mishra');
        expect(adminQuizCreate(authUserId.authUserId, 'Test Quiz!', 'A test quiz')).toEqual(ERROR);
    });

    test('Invalid description - too long', () => {
        const authUserId = adminAuthRegister('swastik@example.com', 'password123', 'swastik', 'mishra');
        const longDescription = 'A'.repeat(101);
        expect(adminQuizCreate(authUserId.authUserId, 'Test Quiz', longDescription)).toEqual(ERROR);
    });

    test('Successfully create a quiz', () => {
        const authUserId = adminAuthRegister('swastik@example.com', 'password123', 'swastik', 'mishra');
        const result = adminQuizCreate(authUserId.authUserId, 'Test Quiz', 'A test quiz');
        expect(result).toEqual({ quizId: expect.any(Number) });
        
        const quizzes = adminQuizList(authUserId.authUserId);
    ...
    test('Create quiz with same name for different users', () => {
        ...
        const quiz1 = adminQuizCreate(user1.authUserId, 'Common Quiz', 'A quiz');
        const quiz2 = adminQuizCreate(user2.authUserId, 'Common Quiz', 'Another quiz');

        expect(quiz1.quizId).not.toBe(quiz2.quizId);

        const quizzes1 = adminQuizList(user1.authUserId);
        const quizzes2 = adminQuizList(user2.authUserId);
